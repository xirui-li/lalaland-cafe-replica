...
The owner’s latest instruction is to show yogurt photography in its original colors, without a filter. This applies to the product gallery, product details, and search results. The blue treatment in `src/ThemeFilters.tsx` remains on the main video, storefront photo, and brand illustration. Logo masks and page colors retain the Pantone palette.
